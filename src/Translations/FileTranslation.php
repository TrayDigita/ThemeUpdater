...
class FileTranslation extends AbstractTranslation
{
     ...
     * @param string $file
     */
    public function __construct(ThemeTranslations $translations, string $file = null)
    {
    ...
    public function getMetaData(): TranslationMetadata
    {
        ...
        $defaults = [];
        $metadata = $this->metadata->defaults;
        ...
        if ($ext === '.mo') {
            ...
            // json
            $data = @file_get_contents($this->file, false, null, 0, KB_IN_BYTES * 10);
            if (is_string($data)) {
